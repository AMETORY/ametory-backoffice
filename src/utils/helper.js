...
export const durationSecondToHumanReadable = (durationSecond) => {

    const hours = Math.floor(durationSecond / (60 * 60));
    const minutes = Math.floor((durationSecond % (60 * 60)) / 60);
    const seconds = durationSecond % 60;

    const hoursString = hours > 0 ? `${hours} jam ` : '';
    const minutesString = minutes > 0 ? `${minutes} menit ` : '';
    const secondsString = seconds > 0 ? `${seconds} detik` : '';
